class WindowMgr {
	//舞台	 
	private static _stage: egret.DisplayObjectContainer;
	//当前视图对象列表，最后打开的窗口在数组末位
	private static _nowWindows: IView[] = [];
	//视图映射
	private static _viewClsMap: any = {};
	//模糊背景
	private static _blur: egret.Shape = null;
	//遮挡信息
	private static _blurInfo: any[] = [];

	/**
	 * 初始化管理器(必须)
	 */
	public static init(stage: egret.DisplayObjectContainer): void {
		this._stage = stage;
		this._blur = RectUtil.createRectShape(stage.width, stage.height, 0, -1, 0);
		this._blur.addEventListener(egret.TouchEvent.TOUCH_TAP, this.onBlurTouchTap, this);
		this._blur.touchEnabled = true;
	}

	public static resizeBlur(): void {
		this._blur.graphics.beginFill(0, 1);
		this._blur.graphics.drawRect(0, 0, this._stage.width, this._stage.height);
		this._blur.graphics.endFill();
	}

	private static onBlurTouchTap(e: egret.TouchEvent): void {
		NoticeManager.sendNoticeQuick(Notice.WINDOW_BLUR_TOUCHED, this._nowWindows[this._nowWindows.length - 1]);
	}

	/**
	 * 注册一个场景
	 */
	public static regist(viewName: string, cls: any): void {
		this._viewClsMap[viewName] = cls;
	}


	/**
	 * 打开窗口
	 * @param viewName 打开窗口的名称
	 * @param data 传递到打开窗口的数据。默认为null
	 * @param blur 是否在窗口下显示遮罩。该值只确定是否在窗口下可视的显示遮罩。并不影响触摸事件的向下传递。默认为true
	 * @param closeOthers 是否在打开窗口同时关闭其它所有已打开的窗口。默认为false
	 * @returns 返回打开窗口的实例化对象
	 */
	public static open(viewName: string, data: any = null, blur: boolean = true, closeOthers: boolean = false): IView {
		let cls = this._viewClsMap[viewName];
		if (null == cls) {
			egret.error(`打开的窗口[${viewName}]不存在`);
			return;
		}


		if (closeOthers) {
			this.closeAll();
		}

		this._stage.addChild(this._blur);
		this.resizeBlur();
		this._blur.alpha = blur ? 0.7 : 0;


		let newView: any = new cls();
		newView.onInit(data);
		this._stage.addChild(newView);
		let lastIdx = this._nowWindows.length;
		this._nowWindows.push(newView);
		this._blurInfo[lastIdx] = blur;
		newView.onShow(data);
		return newView;
	}

	/**
	 * 关闭对应的窗口
	 * @param target 要关闭的对象
	 */
	public static close(target: IView): void {
		if (null == target) {
			return;
		}

		let idx = this._nowWindows.indexOf(target);
		if (idx > -1) {
			//从列表移除
			this._nowWindows.splice(idx, 1);
			this._blurInfo.splice(idx, 1);
			target.onDispose();
			this._stage.removeChild(<any>target);
			delete this._blurInfo[<any>target];
		}

		let windowCount = this._nowWindows.length;
		if (windowCount > 0) {
			this._stage.addChild(this._blur);
			let topWindow = <any>this._nowWindows[windowCount - 1];
			this._stage.swapChildren(this._blur, topWindow);
			this._blur.alpha = this._blurInfo[windowCount - 1] ? 0.7 : 0;
		}
		else if (0 == windowCount && null != this._blur.parent) {
			this._blur.parent.removeChild(this._blur);
		}
	}

	/**
	 * 关闭所有窗口
	 */
	public static closeAll(): void {
		let size = this._nowWindows.length;
		while (--size > -1) {
			let target = this._nowWindows[size];
			target.onDispose();
			this._stage.removeChild(<any>target);
		}
		this._nowWindows.length = 0;
		if (this._blur.parent != null) {
			this._blur.parent.removeChild(this._blur);
		}
	}
}